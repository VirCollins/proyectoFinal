...
const AsignarModulos = () => {
    ...
    const fetchUsuarios = async () => {
        try {
            const response = await fetch("http://192.168.100.122/ProyectoFinalBacken/usuario.php");
    ...
    const fetchModulos = async (usuario_id, usuario_nombre) => {
        try {
            const response = await fetch(`http://192.168.100.122/ProyectoFinalBacken/Acceso.php?usuario_id=${usuario_id}`);
            const result = await response.json();
            setModulos(result);
    ...
    const toggleAcceso = async (usuario_id, modulo_codigo, acceso_estado) => {
        try {
            ...
            const response = await fetch("http://192.168.100.122/ProyectoFinalBacken/Acceso.php", {
                method: "POST",
